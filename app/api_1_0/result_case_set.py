...
def find_result_case_set():
    ...
    case_success = db.session.query(func.count(func.distinct(ResultDetail.case_id))).filter_by(project_id=project_id, report_id=report_id, case_set_id=case_set_id, case_exec_status = 1).all()[0][0]
    case_faile = db.session.query(func.count(func.distinct(ResultDetail.case_id))).filter_by(project_id=project_id,report_id=report_id,case_set_id=case_set_id,case_exec_status=0).all()[0][0]
    ...
    _data = {'caseTimeStartAt':case_time_start_at,
